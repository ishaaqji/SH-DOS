...
export async function GET(
  ...
  try {
    const audit = await api.contentAudit(workspaceId, contentId);
    return NextResponse.json({ audit });
  } catch (err) {
    if (err instanceof ApiError) {
      return NextResponse.json({ error: { code: err.code, message: err.message } }, { status: err.status });
    }
    return NextResponse.json({ error: { code: "INTERNAL_ERROR", message: "Failed to load audit history" } }, { status: 500 });
  }
}
